...
const outPath = resolve(outDir, "conejo.glb");
const importFile = (relative) => import(pathToFileURL(resolve(repoRoot, relative)).href);

const { createConejoRabbitModel } = await importFile("src/three/createConejoRabbitModel.ts");
const { applyConejoPalette, applyConejoProportions } = await importFile("src/three/conejoModelConfig.ts");
const { buildConejoClips } = await importFile("src/three/conejoClips.ts");

const model = createConejoRabbitModel({ castShadow: true, receiveShadow: true });
applyConejoPalette(model);
applyConejoProportions(model);
const clips = buildConejoClips(model, 1);
